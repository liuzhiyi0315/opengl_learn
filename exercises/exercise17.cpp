...
void exercise17(GLFWwindow* window) {
    struct ShaderInfo
    {
        unsigned int id;
    ...
    auto vertexShaderSource = ParseShader("../res/shader/advanced/e17/e17_vertex.shader");

    std::string frag_name[4] = {"red", "green", "blue", "yellow"};

    ShaderInfo color4_shaders[4];
    char fragmentSourcePath[64];

    for (auto i = 0; i < 4; i ++ ) {
        auto vertexShader = createShader(GL_VERTEX_SHADER, vertexShaderSource);

        std::sprintf(fragmentSourcePath, "../res/shader/advanced/e17/e17_fragment_%s.shader", frag_name[i].c_str());
	...
	{       
        ...
        for (int i = 0; i < 4; i ++) {
            ...
            cube_model = glm::rotate(cube_model, (float)glfwGetTime() * glm::radians(-30.0f), glm::vec3(0.5f, 1.0f, 0.0f));
