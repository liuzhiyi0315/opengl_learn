...
vec3 CalcLight(Light light, vec3 normal, vec3 viewDir, vec3 fragPos)
{
    ...
    // 合并结果
    vec3 ambient  = light.part.ambient  * vec3(texture(material.diffuse, TexCoord2));
    vec3 diffuse  = light.part.diffuse  * diff * vec3(texture(material.diffuse, TexCoord2));
    vec3 specular = light.part.specular * spec * vec3(texture(material.specular, TexCoord2));
